...
- `remove_constant_reward_groups`：是否丢弃并重采样 reward 全相同的组。`false` 会保留这些组；其 advantage 全为零，因此不产生 policy-gradient 信号。
- `max_rollout_attempts`：启用 constant-group 重采样时的最多尝试次数。
