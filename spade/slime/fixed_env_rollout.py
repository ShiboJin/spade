...
Called when --spade-mode fixed_env. Uses external environments (GEM/RLVE)
...
from spade.core.difficulty_controller import create_difficulty_controller
from spade.core.envs.env_adapter import EnvironmentAdapter
...
from spade.slime.trajectory_converter import trajectory_to_slime_sample
...
def _parse_env_sources(source_specs: List[str], envduels_ids_file=None, envduels_seed=42) -> List[EnvironmentAdapter]:
    ...
    if rlve_names:
        try:
            from spade.core.envs.rlve_adapter import RlveEnvironmentAdapter
    ...
    if gem_tasks:
        try:
            from spade.core.envs.gem_adapter import GemEnvironmentAdapter
...
def _pre_generate_games(args: Namespace) -> SyntheticGameAdapter:
    ...
    Creates a SpadeOrchestrator, generates N games, discards env trajectories,
    and returns a SyntheticGameAdapter wrapping the saved game files.

    Args:
        args: Slime Namespace with SPADE configuration
    ...
    from spade.core.learning_potential import LearningPotential
    from spade.core.orchestrator import SpadeOrchestrator

    num_games = args.spade_pre_generate_games
    difficulty = args.spade_pre_generate_difficulty

    # Determine skills
    skills = args.spade_pre_generate_skills
    if not skills:
    ...
    gen_config = SpadeConfig(
        env_temperature=args.spade_env_temperature,
        env_top_p=args.spade_env_top_p,
        env_top_k=args.spade_env_top_k,
        env_max_tokens=args.spade_env_max_tokens,
    ...
    learning_potentials = {
        skill: LearningPotential(
            gamma_fast=args.spade_gamma1,
            gamma_slow=args.spade_gamma2,
        )
        for skill in skills
    }
    ...
    games_dir = Path(args.spade_games_dir)
...
def _get_orchestrator(args: Namespace) -> FixedEnvOrchestrator:
    ...
    if args.spade_pre_generate_games > 0:
    ...
    spade_config = SpadeConfig(
        ...
        max_context_length=args.spade_max_context_length,
        gamma=args.spade_gamma,
        reward_normalization=args.spade_reward_normalization,
        game_baseline_decay=args.spade_game_baseline_decay,
    )
    ...
    if spade_config.reward_normalization == "ema_baseline":
        baseline_tracker = GameBaselineTracker(decay=spade_config.game_baseline_decay)

    # Choose between fixed pool (ablation) and adaptive difficulty
    fixed_pool = None
    if args.spade_fixed_pool_size > 0:
        fixed_pool = _create_fixed_pool(
            adapters=adapters,
            pool_size=args.spade_fixed_pool_size,
            max_difficulty=args.spade_fixed_pool_max_difficulty,
            seed=args.spade_fixed_pool_seed,
        )
        logger.info(
    ...
    difficulty_controller = create_difficulty_controller(
        variant=args.spade_difficulty_variant,
        tau_acc=args.spade_difficulty_tau_acc,
        tau_num=args.spade_difficulty_tau_num,
        d_delta=args.spade_difficulty_d_delta,
    ...
    _FIXED_ENV_ORCHESTRATOR = FixedEnvOrchestrator(
        ...
        env_regeneration_interval=args.spade_game_regeneration_interval,
        fixed_pool=fixed_pool,
        no_replacement=True,  # Always use no-replacement for fixed pools
        same_problem_groups=args.spade_fixed_env_same_problem,
    )

    return _FIXED_ENV_ORCHESTRATOR


def spade_fixed_env_rollout(
    args: Namespace,
    rollout_id: int,
    ...
    if evaluation:
        ...
        if args.spade_gem_eval_config:
            from spade.slime.gem_eval_rollout import spade_gem_eval_rollout
            return spade_gem_eval_rollout(args, rollout_id, data_source)
